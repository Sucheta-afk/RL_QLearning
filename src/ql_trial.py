...
env_row = 12
env_col = 12
goal_x1 = 8  # int(input("Please enter goal x coordinate: "))
goal_y1 = 4  # int(input("Please enter goal y coordinate: "))
goal_x2 = 10 
goal_y2 = 8
goal_x3 = 4 
goal_y3 = 8 
...
obstacles = [[1,5],[2,5],[2,3],[2,2],[2,1],[4,1],[4,3],[4,4],[4,5],[5,1],[5,2],[5,4]]

# assign reward value to goal
for i in range(len(q_values)):
    for j in range(len(q_values[0])):
        rewards[i][j] = - get_euclidian_dist(i,j,goal_x1,goal_y1)
...
def is_final_state(row, col):
    if row == goal_x1 and col == goal_y1:
        return True
    else:
        return False
...
def Goto_goal(x_goal1, y_goal1):
    ...
    ang_dist = math.atan2((y_goal1-y1),(x_goal1-x1))
...
def spawn_target(x, y):
    ...
    spawn_target_service(goal_x1, goal_y1, 0, 'target')
    spawn_target_service(goal_x1, goal_y2, 0, 'target')
...
for i in range(len(q_values)):
    for j in range(len(q_values[0])):
        rewards[i][j] = - get_euclidian_dist(i,j,goal_x2,goal_y2)
...
def is_final_state(row, col):
    if row == goal_x2 and col == goal_y2:
        return True
    else:
        return False
...
def Goto_goal(x_goal2, y_goal2):
    ...
    distance = abs(math.sqrt(((x_goal2-x1)**2)+((y_goal2-y1)**2)))

    ang_dist = math.atan2((y_goal2-y1),(x_goal2-x1))
    while (abs(ang_dist-theta1)>0.01):
        Phi = 2
        ang_dist = math.atan2((y_goal2-y1),(x_goal2-x1))
...
'''
def spawn_target(goal_x1, goal_y1, goal_x2, goal_y2):
    # Spawn the target
    spawn_target_service = rospy.ServiceProxy('/spawn', Spawn)
 '''  
